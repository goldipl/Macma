<div class="offertspage_container step_four">
    <div class="container">
        <div class="top-title-box">
            <div class="top-title-box__left">
                <h1>Ofertownik</h1>
            </div>
            <div class="top-title-box__right">
                <a href="#" class="help-box">
                    <img src="./assets/icons/offertspage/question-octagon.svg" alt="question icon" width="14" height="14">
                    <span>Pomoc</span>
                </a>
            </div>
        </div>
        <div class="offertspage-set-prices">
            <div class="offertspage-step-table-container">
                <table class="offertspage-products-table">
                    <tbody>
                        <tr>
                            <th>LP.</th>
                            <th>Produkt</th>
                            <th>Opis produktu</th>
                            <th>Zapisz</th>
                            <!-- Product 01 -->
                        <tr>
                            <td>
                                <b>1</b>
                            </td>
                            <td>
                                <div class="table-product-box">
                                    <div class="table-product-box__top">
                                        <p>Długopis metalowy</p>
                                    </div>
                                    <div class="table-product-box__bottom">
                                        <div class="table-product-box__bottom--left">
                                            <img src="./assets/img/offertspage/pen.jpg" alt="img" width="166" height="166">
                                        </div>
                                        <div class="table-product-box__bottom--right">
                                            <p>Kod: <span>1333903</span>
                                            </p>
                                            <p>Kolor: <span>czarny</span>
                                            </p>
                                            <p>Marka: <span>Basic</span>
                                            </p>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div class="table-middle-box">
                                    <div class="table-middle-buttons">
                                        <div class="table-middle-buttons__slot">
                                            <button class="white-btn"> Wczytaj domyślną nazwę <img src="./assets/icons/common/buttons/button_orange_arrow.svg" alt="arrow icon" width="18" height="18"></button>
                                            <input type="text" class="form-input">
                                        </div>
                                        <div class="table-middle-buttons__slot">
                                            <button class="white-btn"> Wczytaj domyślny kod <img src="./assets/icons/common/buttons/button_orange_arrow.svg" alt="arrow icon" width="18" height="18"></button>
                                            <input type="text" class="form-input">
                                        </div>
                                        <div class="table-middle-buttons__slot">
                                            <button class="white-btn"> Wczytaj domyślny opis <img src="./assets/icons/common/buttons/button_orange_arrow.svg" alt="arrow icon" width="18" height="18"></button>
                                            <textarea class="form-textarea"></textarea>
                                        </div>
                                    </div>
                                    <div class="table-middle-checkboxes">
                                        <div class="table-middle-checkboxes__slot">
                                            <label class="custom-checkbox-container">Kolor <input type="checkbox" class="custom_input">
                                            <span class="custom-checkbox-checkmark"></span>
                                            </label>
                                        </div>
                                        <div class="table-middle-checkboxes__slot">
                                            <label class="custom-checkbox-container">Materiał <input type="checkbox" class="custom_input">
                                            <span class="custom-checkbox-checkmark"></span>
                                            </label>
                                        </div>
                                        <div class="table-middle-checkboxes__slot">
                                            <label class="custom-checkbox-container">Opakowanie <input type="checkbox" class="custom_input">
                                            <span class="custom-checkbox-checkmark"></span>
                                            </label>
                                        </div>
                                        <div class="table-middle-checkboxes__slot">
                                            <label class="custom-checkbox-container">Waga <input type="checkbox" class="custom_input">
                                            <span class="custom-checkbox-checkmark"></span>
                                            </label>
                                        </div>
                                        <div class="table-middle-checkboxes__slot">
                                            <label class="custom-checkbox-container">Rozmiar <input type="checkbox" class="custom_input">
                                            <span class="custom-checkbox-checkmark"></span>
                                            </label>
                                        </div>
                                        <div class="table-middle-checkboxes__slot">
                                            <label class="custom-checkbox-container">Znakowanie <input type="checkbox" class="custom_input">
                                            <span class="custom-checkbox-checkmark"></span>
                                            </label>
                                        </div>
                                        <div class="table-middle-checkboxes__slot">
                                            <label class="custom-checkbox-container">Rozmiar znakowania <input type="checkbox" class="custom_input">
                                            <span class="custom-checkbox-checkmark"></span>
                                            </label>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <div class="orange-btn save-btn">Zapisz <img src="./assets/icons/common/buttons/button_white_arrow.svg" alt="arrow icon" width="18" height="18"></div>
                            </td>
                        </tr>
                </table>
            </div>
            <div class="bottom-offertspage-box-with-btn">
                <div class="bottom-offertspage-button">
                    <a href="#" class="white-btn prev left-btn">
                        <img src="./assets/icons/common/buttons/button_orange_arrow.svg" alt="arrow icon" width="18" height="18"> Powrót </a>
                </div>
                <div class="bottom-offertspage-button right-btn">
                    <a href="#" class="orange-btn next">Dalej <img src="./assets/icons/common/buttons/button_white_arrow.svg" alt="arrow icon" width="18" height="18">
                    </a>
                </div>
            </div>
        </div>
    </div>
</div>